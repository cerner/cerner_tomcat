  ...
  module Resources
    module Template
      class Resource < Chef::Resource::Template
        ...
        provides(:cerner_tomcat_template)
        actions(:create, :create_if_missing, :delete, :touch)

        def initialize(*args)
          super
          @provider = CernerTomcat::Resources::Template::Provider
          # For older Chef (< 12.4)
          @resource_name = :cerner_tomcat_template
        end
      end

      class Provider < Chef::Provider::Template
        include CernerTomcat::FileMixin
      end
    end
  end
